...
add_action('wp_ajax_toggle_sub_group', 'handle_toggle_sub_group');
function handle_toggle_sub_group()
{
  ...
  $status   = sanitize_text_field($_POST['status']); // active or inactive

  if (!$group_id) {
    wp_send_json_error('Invalid group ID.');
  }

  // If turning ON → make all others inactive first
  if ($status === 'active') {
    ...
    foreach ($other_groups as $group) {
      update_field('group_status', 'inactive', $group->ID);
    }
  }

  // Update this group
  update_field('group_status', $status, $group_id);

  wp_send_json_success("Subgroup status set to {$status}.");
}
...
function render_subgroups_by_issue_and_gender($issue, $gender)
{
  ...
  foreach ($groups as $index => $group) {
    ...
    $group_status = get_field('group_status', $post_id);
    $status_class = (strtolower($group_status) === 'active') ? 'active-group' : 'inactive-group';
    $status_icon = ($status_class === 'active-group') ? 'bi-check-circle-fill' : 'bi-circle-fill';
    ...
    // Collapsible button (do NOT let it take full width)
    ...
    echo '<span>Sub Group ' . esc_html($group_number) . " (Max Members: " . esc_html($max_members) . ")</span>";
    echo '</button>';

    // Status toggle
    $checked = (strtolower($group_status) === 'active') ? 'checked' : '';
    $toggle_class = strtolower($group_status) === 'active' ? 'toggle-on' : 'toggle-off';

    echo <<<HTML
<div class="d-flex align-items-center ms-3">
    <span class="group-status {$status_class} {$toggle_class}">
        <i class="bi {$status_icon} me-1"></i>
        <span class="status-text">{$group_status}</span>
    </span>
    <label class="custom-toggle ms-2 mb-0">
        <input type="checkbox" class="toggle-group-status"
               data-group-id="{$post_id}"
               data-issue="{$issue}"
               data-gender="{$gender}" {$checked}>
...
HTML;
    ...
    echo '<p><strong>Session Start Date:</strong> ' . esc_html($session_start_date) . '</p>';
    echo '<p><strong>Session Expiry Date:</strong> ' . esc_html($session_expiry_date) . '</p>';
    ...
    if ($members) {
      ...
      echo '<tr><th>#</th><th>Name</th><th>DOB</th><th>NID/Passport</th><th>Country</th><th>Reg Date</th></tr>';
      echo '</thead><tbody>';

      $i = 1;
      foreach ($members as $member) {
        $dob = get_field('birth_date', 'user_' . $member->ID);
        ...
        $nid = get_field('nid_no', 'user_' . $member->ID); // Get the NID

        // Determine which ID to display
        if (!empty($nid)) {
          $id_to_display = $nid;
        } else {
          $id_to_display = $passport;
        }
...
{
  // Security check (optional, you can add nonce verification later)
  ...
  // 1. Set previous subgroups (same issue_type and gender) to "inactive"
  $args = [
    'post_type' => 'therapy_group',
    'post_status' => 'publish',
    'posts_per_page' => -1,
  ...
  $new_post = array(
    'post_title'    => ucfirst($issue_type) . ' - ' . ucfirst($gender) . ' Group',
    'post_type'     => 'therapy_group', // <-- your custom post type
    'post_status'   => 'publish',
  );
  ...
  update_field('group_status', 'active', $post_id); // default status
  update_field('group_number', count($old_groups) + 1, $post_id);
  ...
  notify_waiting_list_users($issue_type, $gender);

  wp_send_json_success('Group created successfully!');
}

function notify_waiting_list_users($issue, $gender)
{
  global $wpdb;
...
// 2. Callback function to render the dashboard content
...
?>
  <div class="wrap">
    ...
    <div class="tab-content">
      ...
      <!-- Add more tab panes like above for Depression, Grief, Relationship -->
    </div>
  </div>
    ...
    <div class="modal-dialog">
      <form id="createSubGroupForm">
        <div class="modal-content">
          <div class="modal-header" style="background:#D58F76; color:white;">
  ...
  <script>
      ...
      fetch('<?php echo admin_url("admin-ajax.php"); ?>', {
        ...
        .then(response => {
          ...
          if (response.success) {
            ...
            // Optional: close modal and reload
    ...
    document.querySelectorAll('.toggle-group-status').forEach(toggle => {
      toggle.addEventListener('change', function() {
        ...
        data.append('action', 'toggle_sub_group');
...
function get_unscheduled_subgroups($issue_type, $gender)
{
  ...
  foreach ($groups as $group) {
    ...
    if (!$has_schedule || !$has_upcoming_meetings) {
      ...
      $group_status = get_field('group_status', $group->ID) ?: 'inactive';
...
function ajax_get_therapy_scheduled_meetings()
{
  ...
  // Get all scheduled meetings (only upcoming ones with valid schedules)
  ...
  if ($wpdb->last_error) {
    ...
    error_log('Therapy Scheduling SQL Query: SELECT m.*, s.issue_type, s.gender FROM ' . $meetings_table . ' m LEFT JOIN ' . $schedules_table . ' s ON m.schedule_id = s.id');
  }
  ...
  if (empty($meetings)) {
    error_log('Therapy Scheduling: No meetings found in database');
    wp_send_json_success([]);
    return;
  }
  ...
  error_log('Therapy Scheduling: Successfully fetched ' . count($meetings) . ' meetings');
  wp_send_json_success($meetings);
}

// SEND MEETING NOTIFICATION EMAIL
function send_therapy_meeting_notification($meeting_id)
{
  global $wpdb;
...
function process_therapy_meeting_notifications()
{
  ...
  foreach ($meetings as $meeting) {
    send_therapy_meeting_notification($meeting['id']);
  }
}
...
function ajax_send_therapy_notification_now()
{
  ...
  $result = send_therapy_meeting_notification($meeting_id);
    ...
    <div class="therapy-scheduling-wrapper">
      ...
      <div class="tab-content">
        ...
        <div class="tab-pane fade" id="bookedSchedule" role="tabpanel">
          <div class="card">
            <div class="card-header"><i class="bi bi-calendar-week me-2"></i>Upcoming Scheduled Meetings</div>
            <div class="card-body">
        ...
        jQuery(document).ready(function($) {
            ...
            $.post(ajaxUrl, {
              ...
              if (response.success && response.data.length > 0) {
                ...
                response.data.forEach(function(group) {
                  ...
                  html += `<div class="subgroup-card" data-group-id="${group.id}">
          ...
          function loadScheduledMeetings() {
            ...
            $.post(ajaxUrl, {
              ...
              console.log('Therapy Meetings Response:', response);
              ...
              if (response.success) {
                if (response.data && response.data.length > 0) {
                  ...
                  response.data.forEach(function(m) {
                    ...
                    const notified = m.notification_sent == 1 ? '<span class="badge bg-success">Yes</span>' : '<span class="badge bg-secondary">No</span>';
                    const zoomLink = m.zoom_join_url ? `<a href="${m.zoom_join_url}" target="_blank" class="zoom-link">${m.zoom_join_url}</a>` : '<span class="text-muted">N/A</span>';
                    ...
                    <td>${notified}</td>
                    <td>
                      <button class="btn btn-sm btn-outline-primary send-notification-btn" data-id="${m.id}" ${m.notification_sent==1?'disabled':''}>
                  ...
                  $('#meetingsTableBody').html('<tr><td colspan="9" class="text-center text-muted">No scheduled meetings found. Create a schedule in the "Schedule Booking" tab.</td></tr>');
                }
              } else {
                console.error('Error loading meetings:', response);
